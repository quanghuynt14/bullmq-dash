...
fields (e.g. legacy `host`/`port`) are rejected as `CONFIG_ERROR`.
`cacheTtlMs` controls the SQLite observation-cache TTL and defaults to 24 hours;
legacy `retentionMs` is accepted as an alias. Strings of the form `${VAR_NAME}`
interpolate an environment variable as the **whole value** (partial substitution
is intentionally not supported). Unset references are a hard `CONFIG_ERROR`
