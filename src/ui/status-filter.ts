...
import type { JobStatus } from "../data/jobs.js";
...
const STATUS_OPTIONS: { key: string; status: JobStatus; label: string }[] = [
...
export function updateStatusFilter(elements: StatusFilterElements, currentStatus: JobStatus): void {
  const { text } = elements;

  // Build parts with separators between them
  const parts: (StyledText | string)[] = [t`Status: `];
...
export function getStatusFromKey(key: string): JobStatus | null {
