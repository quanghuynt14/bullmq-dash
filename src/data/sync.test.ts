...
mock.module("./jobs.js", () => ({
  getAllJobIds: async function* (_ctx: unknown, _queueName: string) {
...
afterEach(async () => {
  ...
  try {
    ...
    // closeSqliteDb-mid-sync test already closed it
  }
...
describe("syncQueue", () => {
  ...
  it("sets result.error and resets syncInProgress on failure", async () => {
    ...
    const first = await syncQueue(ctx, "q");
    ...
    const second = await syncQueue(ctx, "q");
  ...
  it("does not soft-delete jobs polling just inserted that staging missed", async () => {
    ...
    const row = getJobFromDb(ctx, "q", "99");
  ...
  it("soft-deletes jobs missing from Redis instead of hard-deleting", async () => {
    ...
    const gone = getJobFromDb(ctx, "q", "2", { view: "all" });
    ...
    expect(getJobFromDb(ctx, "q", "2")).toBeNull();
  });
  ...
  it("throws when a soft-deleted id reappears in Redis", async () => {
    ...
    softDeleteJobsByIds(ctx, "q", ["2"], 100);
    ...
    const thrown = await syncQueue(ctx, "q").catch((e: unknown) => e);
    ...
    const row = getJobFromDb(ctx, "q", "2", { view: "all" });
  ...
  it("lists every offending id in the resurrection error message", async () => {
    ...
    softDeleteJobsByIds(ctx, "q", ["2", "3"], 100);
  ...
  it("aborts with a clear error when the SQLite connection is swapped mid-sync", async () => {
    let releaseGate!: () => void;
    mockState.gate = new Promise<void>((resolve) => {
      releaseGate = resolve;
    });
    ...
    // Lock must be released so a subsequent sync can proceed against the
    // new connection.
    mockState.gate = null;
...
describe("fullSync", () => {
  it("aggregates per-queue errors without aborting", async () => {
    ...
    let call = 0;
    mock.module("./jobs.js", () => ({
      getAllJobIds: async function* (_ctx: unknown, _q: string) {
        call++;
  ...
  it("rethrows resurrected ids after syncing remaining queues without compacting evidence", async () => {
    upsertJobs(ctx, "bad", [{ id: "ghost", name: "ghost", state: "completed", timestamp: 1 }]);
    upsertJobs(ctx, "good", [{ id: "expired", name: "old", state: "completed", timestamp: 1 }]);
    softDeleteJobsByIds(ctx, "bad", ["ghost"], 1);
    softDeleteJobsByIds(ctx, "good", ["expired"], 1);

    ctx.config.retentionMs = 10;

    mockState.queues = ["bad", "good"];
    mock.module("./jobs.js", () => ({
      getAllJobIds: async function* (_ctx: unknown, q: string) {
        if (q === "bad") yield [{ id: "ghost", state: "completed" }];
        if (q === "good") yield [{ id: "live", state: "active" }];
      },
    }));

    await expect(fullSync(ctx)).rejects.toThrow(/bad: .*resurrect/i);
    ...
    expect(getJobFromDb(ctx, "bad", "ghost", { view: "all" })!.removed_at).toBe(1);
    expect(getJobFromDb(ctx, "good", "live")).not.toBeNull();
    // Compaction is skipped for the whole cycle when an invariant fails.
    expect(getJobFromDb(ctx, "good", "expired", { view: "all" })).not.toBeNull();
  });
  ...
  it("compacts rows past retentionMs once after all queues reconcile", async () => {
    ...
    softDeleteJobsByIds(ctx, "q1", ["expired"], 1);
    softDeleteJobsByIds(ctx, "q2", ["expired"], 1);

    ctx.config.retentionMs = 10; // anything stamped > 10ms ago is purged
    ...
    expect(getJobFromDb(ctx, "q1", "expired", { view: "all" })).toBeNull();
    expect(getJobFromDb(ctx, "q2", "expired", { view: "all" })).toBeNull();
    expect(getJobFromDb(ctx, "q1", "live")).not.toBeNull();
    expect(getJobFromDb(ctx, "q2", "live")).not.toBeNull();
  });
});
