...
- **Socket score audit added.** `bun run security:score` can manually score a published package via the pinned Socket CLI (`@socketsecurity/cli@1.1.94`) and compare the result against an accepted-alert allowlist. It is intentionally not part of the publish workflow because Socket scoring can lag or fail after npm accepts an immutable package version.
- **CI supply chain hardened.** GitHub Actions in CI and publish workflows are pinned by commit SHA, Dependabot tracks the `github-actions` ecosystem, and `bun run security:verify-workflows` rejects mutable action refs, `pull_request_target`, direct `${{ github.event.* }}` command interpolation, missing source-control/lockfile/workflow/package verifier steps, disabled npm lifecycle scripts, publish secrets, and publish workflows that omit provenance publishing.
