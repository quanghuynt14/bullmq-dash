...
- **Socket score gate added.** `bun run security:score` scores the published package via the pinned Socket CLI (`@socketsecurity/cli@1.1.94`) and compares the result against an accepted-alert allowlist that includes capabilities inherent to a Redis monitoring tool (`networkAccess`, `urlStrings`, `filesystemAccess`, `envVars`), Socket's transient `recentlyPublished` window, and the build-shape alerts inherent to the published graph (`hasNativeCode`, `minifiedFile`). Risk-signal alert types (`debugAccess`, `gptAnomaly`, `newAuthor`, `nonpermissiveLicense`, `obfuscatedFile`, `shellAccess`, `unmaintained`, `usesEval`) are intentionally not pre-accepted; the gate surfaces them on first publish for per-incident triage and either revert or explicit allowlist entry with citation.
